...
# RGB color palette for each segmentation class
BRATS_COLOR_MAP = np.array(
    [
        [0, 0, 0],        # Background — black
        [220, 20, 60],    # NCR/NET   — crimson
        [30, 144, 255],   # ED        — dodger blue
        [50, 205, 50],    # ET        — lime green
    ],
    dtype=np.uint8,
)
...
def mask_to_color(mask):
    """Convert a class-index mask to an RGB color image using the BraTS palette."""
...
def _prepare_background_image(image):
    """
    Normalize a multi-channel MRI slice to [0, 1] for grayscale display.
    Uses the last channel (FLAIR) as the background for overlays.
    """
    if image.ndim == 3:
        base = image[-1]  # FLAIR channel (last of T1, T1ce, T2, FLAIR)
    else:
        base = image
...
def save_segmentation_mask(mask, output_path):
    """Save a class-index segmentation mask as a color PNG image."""
    plt.imsave(output_path, mask_to_color(mask))


def save_class_masks(pred, output_dir, prefix="pred", class_names=None):
    """Save individual binary masks for each foreground tumor class."""
...
def plot_predictions(image, gt, pred, output_path, class_names=None):
    ...
    # Build color legend for foreground classes
    legend_handles = [
        plt.Line2D(
            [0],
            [0],
...
def plot_training_curve(log_csv, output_path=None):
    """
    Plot training and validation curves from the log CSV.
    Creates two panels: loss curves (left) and Dice score curves (right).
    """
